...
/** Resolve a tab label to the heading it should scroll to.
 *
 *  An exact slug match is the fast path, but a tab is a short pill and a
 *  heading is descriptive, so they drift: "A01 Access Control" against
 *  "A01 · Broken Access Control — First American Financial". When they drift the
 *  old lookup returned null and the tab silently did nothing — 20 tabs across 7
 *  chapters were inert, including every tab in the OWASP guide. So fall back to
 *  a prefix match on the normalised text before giving up. */
function resolveHeading(label: string): HTMLElement | null {
  const exact = document.getElementById(ghSlug(label));
  ...
  return (
    headings.find((h) => norm(h.textContent ?? "").startsWith(want)) ??
    headings.find((h) => norm(h.textContent ?? "").includes(want)) ??
    null
  );
}
...
export function ChapterTabs({ tabs }: { tabs: string[] }) {
  ...
  const go = (i: number) => {
    const el = resolveHeading(tabs[i]);
