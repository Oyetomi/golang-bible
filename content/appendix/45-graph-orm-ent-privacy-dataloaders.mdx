...
<ELI5
  ...
  analogy="Fetching one grocery at a time because someone asked for a list of items."
  role="librarian"
>

Load 100 transactions, then display each one's account name. Written naturally, that is one query for the transactions and then one per transaction for its account — 101 queries where 2 would do.

It hides well because the code looks innocent. The second query is usually behind a property access or a resolver, so nothing at the call site says "database". It also passes review and passes tests, because with three test rows the difference is invisible.

GraphQL makes it worse by design, since each field resolves independently and has no idea it is one of a hundred siblings.

A **dataloader** fixes it without changing how the code reads. Instead of issuing a query immediately, each request for an account records the id and waits a moment. At the end of that tick the loader takes every id collected, issues **one** query with `WHERE id IN (...)`, and hands each caller its own result.

101 queries become 2, and the calling code is untouched — it still looks like it is fetching one account at a time.

The tell to watch for: a query count that scales with rows returned rather than with the shape of the request. If asking for twice the data means twice the queries, you have found one.
